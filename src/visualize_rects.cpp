...
#include "detected_objects_visualizer/visualize_rects.hpp"
...
VisualizeRects::VisualizeRects() {
  ...
  if (found_pos != std::string::npos) ros_namespace.erase(found_pos, ros_namespace.length() - found_pos);
  std::cout << ros_namespace << std::endl;
  image_out_topic = ros_namespace + image_out_topic;

  image_filter_subscriber_ = new message_filters::Subscriber<sensor_msgs::Image>(private_nh_, image_src_topic, 1);
  ROS_INFO("[%s] image_src: %s", __APP_NAME__, image_src_topic.c_str());
  detection_filter_subscriber_ =
      new message_filters::Subscriber<autoware_msgs::DetectedObjectArray>(private_nh_, object_src_topic, 1);
  ROS_INFO("[%s] object_src: %s", __APP_NAME__, object_src_topic.c_str());

  detections_synchronizer_ = new message_filters::Synchronizer<SyncPolicyT>(SyncPolicyT(10), *image_filter_subscriber_,
                                                                            *detection_filter_subscriber_);
  detections_synchronizer_->registerCallback(boost::bind(&VisualizeRects::SyncedDetectionsCallback, this, _1, _2));
...
void VisualizeRects::SyncedDetectionsCallback(const sensor_msgs::Image::ConstPtr &in_image_msg,
...
cv::Mat VisualizeRects::ObjectsToRects(cv::Mat in_image,
                                       const autoware_msgs::DetectedObjectArray::ConstPtr &in_objects) {
  ...
  for (auto const &object : in_objects->objects) {
    if (IsObjectValid(object)) {
      ...
      if (rect.x + rect.width >= in_image.cols) rect.width = in_image.cols - rect.x - 1;

      if (rect.y + rect.height >= in_image.rows) rect.height = in_image.rows - rect.y - 1;
      ...
      cv::rectangle(final_image, text_origin + cv::Point(0, baseline),
                    text_origin + cv::Point(text_size.width, -text_size.height), cv::Scalar(0, 0, 0), cv::FILLED,
                    cv::LINE_AA, 0);

      cv::putText(final_image, label, text_origin, font_face, font_scale, cv::Scalar::all(255), thickness, cv::LINE_AA,
                  false);
    }
  }
  return final_image;
...
bool VisualizeRects::IsObjectValid(const autoware_msgs::DetectedObject &in_object) {
  if (!in_object.valid || in_object.width < 0 || in_object.height < 0 || in_object.x < 0 || in_object.y < 0) {
    return false;
  }
  return true;
}  // end IsObjectValid